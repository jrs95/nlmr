  ...
  for (i in 1:q) {
    if (family == "gaussian") {
      if (is.null(covar)) {
        ...
        model <- lm(y[x0q == i] ~ g[x0q == i] + covar[x0q == i, , drop = F])
      }
    ...
    if (family == "binomial") {
      if (is.null(covar)) {
        ...
        model <- glm(y[x0q == i] ~ g[x0q == i] + covar[x0q == i, , drop = F], family = "binomial")
      }
      if (is.na(model$coef[2])) stop("the regression coefficient of the outcome on the instrument in one of the quantiles is missing")
      coef[i] <- model$coef[2]
      coef_se[i] <- summary(model)$coef[2, 2]
    ...
    if (xc_sub) {
      if (family == "gaussian") {
        if (is.null(covar)) {
          ...
          xcoefs <- lm(x[x0q == i] ~ g[x0q == i] + covar[x0q == i, , drop = F])
        }
      ...
      if (family == "binomial") {
        if (is.null(covar)) {
          ...
          xcoefs <- lm(x[(x0q == i & y == 0)] ~ g[(x0q == i & y == 0)] + covar[(x0q == i & y == 0), , drop = F])
        }
        if (is.na(xcoefs$coef[2])) stop("the regression coefficient of the exposure on the instrument in one of the quantiles is missing")
        xcoef_sub[i] <- xcoefs$coef[2]
        xcoef_sub_se[i] <- summary(xcoefs)$coef[2, 2]
