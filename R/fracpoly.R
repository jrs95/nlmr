...
summary.fracpoly_mr <- function(x, ...) {
  ...
  if (!is.null(x$figure)) {
    summ <- list(model = model, powers = powers, n = n, coefficients = coefficients, p_tests = p_tests, p_heterogeneity = p_heterogeneity, figure = x$figure)
  }
...
fracpoly_best <- function(coef, coef_se, xmean, d = 1, pd = 0.05, method = "FE") {
  ...
  for (p1 in powers) {
    ...
    if (is(fp_mod, "try-error") == T) {
      ...
      if (p1 == 0) {
        ...
        if (fp_mod$fit.stats[1, 1] >= suppressWarnings(max(likelihood_d1, na.rm = T))) {
          fp1 <- fp_mod
          p_ML <- p1
        }
      }
    ...
    for (p21 in powers2) {
      ...
      if (is(fp_mod, "try-error") == T) {
        ...
        if (p11 == 0 & p21 == 0) {
          ...
          if (fp_mod$fit.stats[1, 1] >= suppressWarnings(max(likelihood_d2, na.rm = T))) {
            fp2 <- fp_mod
            p1_ML <- p11
            p2_ML <- p21
          }
  ...
  if (d == 1) {
    model <- fp1
    if (length(model$b) != 1) stop("incorrect number of parameters for best fitting fractional polynomial of degree 1")
  }
  if (d == 2) {
    model <- fp2
    if (length(model$b) != 2) stop("incorrect number of parameters for best fitting fractional polynomial of degree 2")
  }
...
fracpoly_boot <- function(y, x, g, covar, q, x0q, xcoef, family = "gaussian", xpos, method = "FE", nboot, d, p_ML = NULL, p1_ML = NULL, p2_ML = NULL) {
  ...
  for (i in 1:nboot) {
    ...
    g_boot <- g[indices]
    if (!is.null(covar)) {
      covar_boot <- as.matrix(covar[indices, , drop = F])
    } else {
      covar_boot <- NULL
    }
    x0qb <- x0q[indices]
    ...
    if (d == 1) {
      ...
      if (p1_ML == p2_ML) {
        if (p2_ML == -1) {
          ...
          x2_boot <- ((p2_ML + 1) * (xmean_boot^p2_ML) * log(xmean_boot) + xmean_boot^p2_ML)
        }
      } else {
        if (p2_ML == -1) {
...
fracpoly_figure <- function(beta, cov, x.min, x.max, family = "gaussian", d = 1, p_ML = NULL, p1_ML = NULL, p2_ML = NULL, ci = "model_se", frac_coef_boot = NULL, ref = mean(x), pref_x = "x", pref_x_ref = "x", pref_y = "y", ci_type = "overall", ci_quantiles = 10, breaks = NULL) {
  if (ci_type == "overall") {
    ...
    if (family == "binomial") {
      ...
      figure <- ggplot(plot.data, aes(x = x))
      figure <- figure + geom_hline(aes(yintercept = 1), colour = "grey") + geom_line(aes(y = yest), color = "black") + geom_line(aes(y = lci), color = "grey") + geom_line(aes(y = uci), color = "grey") + theme_bw() + labs(x = pref_x, y = bquote(.(pref_y) ~ " [" ~ .(pref_x_ref)["ref"] ~ "=" ~ .(round(ref, 2)) ~ "]")) + theme(axis.title.x = element_text(vjust = 0.5, size = 20), axis.title.y = element_text(vjust = 0.5, size = 20), axis.text.x = element_text(size = 18), axis.text.y = element_text(size = 18)) + geom_point(aes(x = x, y = y), data = plot.data.ref, colour = "red", size = 4) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
      if (!is.null(breaks)) {
        figure <- figure + scale_y_continuous(trans = "log", breaks = breaks)
      } else {
  ...
  if (ci_type == "quantile") {
    ...
    if (d == 1) {
      ...
      if (p_ML != -1) {
        plot.data$yest <- beta * plot.data$x^(p_ML + 1) - beta * ref^(p_ML + 1)
        plot.data.est$yest <- beta * plot.data.est$x^(p_ML + 1) - beta * ref^(p_ML + 1)
        if (ci != "bootstrap_per") {
          plot.data$yse <- sqrt((plot.data$x^(p_ML + 1) - ref^(p_ML + 1))^2 * cov)
          plot.data$lci <- plot.data$yest - 1.96 * plot.data$yse
          plot.data$uci <- plot.data$yest + 1.96 * plot.data$yse
        } else {
          boot <- plot.data$x^(p_ML + 1) %*% t(frac_coef_boot) - reprow(ref^(p_ML + 1) %*% t(frac_coef_boot), n = nrow(plot.data))
          plot.data$lci <- rowQuantiles(boot, probs = 0.025)
          plot.data$uci <- rowQuantiles(boot, probs = 0.975)
        }
      }
    ...
    if (d == 2) {
      if (p1_ML == -1 & p2_ML == -1) {
        plot.data$yest <- beta[1] * log(plot.data$x) + beta[2] * log(plot.data$x) * log(plot.data$x) - (beta[1] * log(ref) + beta[2] * log(ref) * log(ref))
        plot.data.est$yest <- beta[1] * log(plot.data.est$x) + beta[2] * log(plot.data.est$x) * log(plot.data.est$x) - (beta[1] * log(ref) + beta[2] * log(ref) * log(ref))
        if (ci != "bootstrap_per") {
          plot.data$yse <- sqrt((log(plot.data$x) - log(ref))^2 * cov[1, 1] + 2 * (log(plot.data$x) - log(ref)) * (log(plot.data$x) * log(plot.data$x) - log(ref) * log(ref)) * cov[1, 2] + (log(plot.data$x) * log(plot.data$x) - log(ref) * log(ref))^2 * cov[2, 2])
          plot.data$lci <- plot.data$yest - 1.96 * plot.data$yse
          plot.data$uci <- plot.data$yest + 1.96 * plot.data$yse
      ...
      if (p1_ML == -1 & p2_ML != -1 & p1_ML != p2_ML) {
        plot.data$yest <- beta[1] * log(plot.data$x) + beta[2] * plot.data$x^(p2_ML + 1) - (beta[1] * log(ref) + beta[2] * ref^(p2_ML + 1))
        plot.data.est$yest <- beta[1] * log(plot.data.est$x) + beta[2] * plot.data.est$x^(p2_ML + 1) - (beta[1] * log(ref) + beta[2] * ref^(p2_ML + 1))
        if (ci != "bootstrap_per") {
          plot.data$yse <- sqrt((log(plot.data$x) - log(ref))^2 * cov[1, 1] + 2 * (log(plot.data$x) - log(ref)) * (plot.data$x^(p2_ML + 1) - ref^(p2_ML + 1)) * cov[1, 2] + (plot.data$x^(p2_ML + 1) - ref^(p2_ML + 1))^2 * cov[2, 2])
          plot.data$lci <- plot.data$yest - 1.96 * plot.data$yse
          plot.data$uci <- plot.data$yest + 1.96 * plot.data$yse
      ...
      if (p1_ML != -1 & p2_ML != -1 & p1_ML == p2_ML) {
        plot.data$yest <- beta[1] * plot.data$x^(p1_ML + 1) + beta[2] * plot.data$x^(p2_ML + 1) * log(plot.data$x) - (beta[1] * ref^(p1_ML + 1) + beta[2] * ref^(p2_ML + 1) * log(ref))
        plot.data.est$yest <- beta[1] * plot.data.est$x^(p1_ML + 1) + beta[2] * plot.data.est$x^(p2_ML + 1) * log(plot.data.est$x) - (beta[1] * ref^(p1_ML + 1) + beta[2] * ref^(p2_ML + 1) * log(ref))
        if (ci != "bootstrap_per") {
          plot.data$yse <- sqrt((plot.data$x^(p1_ML + 1) - ref^(p1_ML + 1))^2 * cov[1, 1] + 2 * (plot.data$x^(p1_ML + 1) - ref^(p1_ML + 1)) * (plot.data$x^(p2_ML + 1) * log(plot.data$x) - ref^(p2_ML + 1) * log(ref)) * cov[1, 2] + (plot.data$x^(p2_ML + 1) * log(plot.data$x) - ref^(p2_ML + 1) * log(ref))^2 * cov[2, 2])
          plot.data$lci <- plot.data$yest - 1.96 * plot.data$yse
          plot.data$uci <- plot.data$yest + 1.96 * plot.data$yse
      ...
      if (p1_ML != -1 & p2_ML != -1 & p1_ML != p2_ML) {
        plot.data$yest <- beta[1] * plot.data$x^(p1_ML + 1) + beta[2] * plot.data$x^(p2_ML + 1) - (beta[1] * ref^(p1_ML + 1) + beta[2] * ref^(p2_ML + 1))
        plot.data.est$yest <- beta[1] * plot.data.est$x^(p1_ML + 1) + beta[2] * plot.data.est$x^(p2_ML + 1) - (beta[1] * ref^(p1_ML + 1) + beta[2] * ref^(p2_ML + 1))
        if (ci != "bootstrap_per") {
          plot.data$yse <- sqrt((plot.data$x^(p1_ML + 1) - ref^(p1_ML + 1))^2 * cov[1, 1] + 2 * (plot.data$x^(p1_ML + 1) - ref^(p1_ML + 1)) * (plot.data$x^(p2_ML + 1) - ref^(p2_ML + 1)) * cov[1, 2] + (plot.data$x^(p2_ML + 1) - ref^(p2_ML + 1))^2 * cov[2, 2])
          plot.data$lci <- plot.data$yest - 1.96 * plot.data$yse
          plot.data$uci <- plot.data$yest + 1.96 * plot.data$yse
    ...
    if (family != "binomial") {
      figure <- ggplot(plot.data, aes(x = x))
      figure <- figure + geom_hline(aes(yintercept = 0), colour = "grey") + geom_line(aes(x = x, y = yest), color = "black", data = plot.data.est) + geom_errorbar(mapping = aes(x = x, ymin = lci, ymax = uci), color = "grey", width = 0.025) + geom_point(aes(y = yest), color = highlight, size = 4) + theme_bw() + labs(x = pref_x, y = bquote(.(pref_y) ~ " [" ~ .(pref_x_ref)["ref"] ~ "=" ~ .(round(ref, 2)) ~ "]")) + theme(axis.title.x = element_text(vjust = 0.5, size = 20), axis.title.y = element_text(vjust = 0.5, size = 20), axis.text.x = element_text(size = 18), axis.text.y = element_text(size = 18)) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
      if (!is.null(breaks)) {
    ...
    if (family == "binomial") {
      ...
      figure <- figure + geom_hline(aes(yintercept = 1), colour = "grey") + geom_line(aes(x = x, y = yest), color = "black", data = plot.data.est) + geom_errorbar(mapping = aes(x = x, ymin = lci, ymax = uci), color = "grey", width = 0.025) + geom_point(aes(y = yest), color = highlight, size = 4) + theme_bw() + labs(x = pref_x, y = bquote(.(pref_y) ~ " [" ~ .(pref_x_ref)["ref"] ~ "=" ~ .(round(ref, 2)) ~ "]")) + theme(axis.title.x = element_text(vjust = 0.5, size = 20), axis.title.y = element_text(vjust = 0.5, size = 20), axis.text.x = element_text(size = 18), axis.text.y = element_text(size = 18)) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
      if (!is.null(breaks)) {
        figure <- figure + scale_y_continuous(trans = "log", breaks = breaks)
      } else {
        ybreaks <- ggplot_build(figure)$layout$panel_params[[1]]$y$breaks
