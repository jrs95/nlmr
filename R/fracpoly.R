...
fracpoly_best <- function(coef, coef_se, xmean, d=1, pd=0.05, method="FE"){
  ...
  for(p1 in powers){
    ...
    if(p1==-3){fp1 <- fp_mod}
      ...
      if(fp_mod$fit.stats[1,1]>=max(likelihood_d1)){fp1 <- fp_mod}
    }
