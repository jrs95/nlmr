...
summary.piecewise_mr <- function(x, ...) {
  ...
  if (!is.null(x$figure)) {
    summ <- list(model = model, n = n, coefficients = coefficients, p_tests = p_tests, p_heterogeneity = p_heterogeneity, figure = x$figure)
  }
...
piecewise_figure <- function(y, x, g, covar = NULL, q = 10, xcoef, coef, x0q, family = "gaussian", nboot = 100, ref = mean(x), pref_x = "x", pref_x_ref = "x", pref_y = "y", ci_quantiles = 10, breaks = NULL) {
  ...
  for (j in 1:ci_quantiles) {
    ...
    for (k in 1:l) {
      if (k == ci_pos) {
        y_mm_quant[j] <- (coef[k - 1] * x_ci - coef[k - 1] * m[k - 1]) + y_mm[k - 1]
      }
    }
  }
  ...
  for (i in 1:nboot) {
    ...
    for (j in 1:q) {
      if (!is.null(covar_boot)) {
        if (family == "gaussian") {
          non_p_boot[i, j] <- (lm(y_boot[x0qb == j] ~ g_boot[x0qb == j] + covar_boot[x0qb == j, , drop = F])$coef[2]) / xcoef
        }
        if (family == "binomial") {
          non_p_boot[i, j] <- (glm(y_boot[x0qb == j] ~ g_boot[x0qb == j] + covar_boot[x0qb == j, , drop = F], family = "binomial")$coef[2]) / xcoef
        }
      } else {
        if (family == "gaussian") {
          non_p_boot[i, j] <- (lm(y_boot[x0qb == j] ~ g_boot[x0qb == j])$coef[2]) / xcoef
        }
        if (family == "binomial") {
          non_p_boot[i, j] <- (glm(y_boot[x0qb == j] ~ g_boot[x0qb == j], family = "binomial")$coef[2]) / xcoef
        }
      }
    }
  }
  ...
  for (i in 1:nrow(non_p_boot)) {
    for (k in 1:l) {
      if (k == ref_pos) {
        y_boot_ref <- (non_p_boot[i, k - 1] * ref - non_p_boot[i, k - 1] * m[k - 1]) + y_boot[i, k - 1]
      }
    }
    for (k in 1:l) {
      if (k == 1) {
      ...
      if (k >= 2) {
        y_boot[i, k] <- (non_p_boot[i, k - 1] * m[k] - non_p_boot[i, k - 1] * m[k - 1]) + y_boot[i, k - 1]
      }
      if (k == ref_pos) {
        y_boot_ref <- (non_p_boot[i, k - 1] * ref - non_p_boot[i, k - 1] * m[k - 1]) + y_boot[i, k - 1]
      }
  ...
  for (j in 1:ci_quantiles) {
    ...
    for (i in 1:nrow(non_p_boot)) {
      ...
      for (k in 1:l) {
        if (k == ci_pos) {
          y_quant_boot <- (non_p_boot[i, k - 1] * x_ci - non_p_boot[i, k - 1] * m[k - 1]) + y_boot[i, k - 1]
        }
      }
  ...
  if (family != "binomial") {
    figure <- ggplot(plot.data, aes(x))
    figure <- figure + geom_hline(aes(yintercept = 0), colour = "grey") + geom_line(aes(x = x, y = y), colour = "black")
    figure <- figure + geom_errorbar(aes(x = x, ymin = y_lci, ymax = y_uci), data = plot.data.1, color = "grey", width = 0.025) + geom_point(aes(x = x, y = y), data = plot.data.1, colour = "black", size = 3) + geom_point(aes(x = x, y = y), data = plot.data.2, colour = "red", size = 3)
    figure <- figure + theme_bw() + labs(x = pref_x, y = bquote(.(pref_y) ~ " [" ~ .(pref_x_ref)["ref"] ~ "=" ~ .(round(ref, 2)) ~ "]")) + theme(axis.title.x = element_text(vjust = 0.5, size = 20), axis.title.y = element_text(vjust = 0.5, size = 20), axis.text.x = element_text(size = 18), axis.text.y = element_text(size = 18)) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
    if (!is.null(breaks)) {
  ...
  if (family == "binomial") {
    ...
    figure <- ggplot(plot.data, aes(x))
    figure <- figure + geom_hline(aes(yintercept = 1), colour = "grey") + geom_line(aes(x = x, y = y), colour = "black")
    figure <- figure + geom_errorbar(aes(x = x, ymin = y_lci, ymax = y_uci), data = plot.data.1, color = "grey", width = 0.025) + geom_point(aes(x = x, y = y), data = plot.data.1, colour = "black", size = 3) + geom_point(aes(x = x, y = y), data = plot.data.2, colour = "red", size = 3)
    figure <- figure + theme_bw() + labs(x = pref_x, y = bquote(.(pref_y) ~ " [" ~ .(pref_x_ref)["ref"] ~ "=" ~ .(round(ref, 2)) ~ "]")) + theme(axis.title.x = element_text(vjust = 0.5, size = 20), axis.title.y = element_text(vjust = 0.5, size = 20), axis.text.x = element_text(size = 18), axis.text.y = element_text(size = 18)) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
    if (!is.null(breaks)) {
